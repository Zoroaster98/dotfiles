...
return {
  ...
  opts = {
    ...
    note_frontmatter_func = function(note)
      ...
      if note.path then
        -- 🔥 FIX: Robust Relative Path Check
        -- This logic handles cases where note.path is a string (causing the nil method error)
        -- or a Plenary object. It converts to a Plenary Path object if needed.
        local path_obj = note.path
        if type(path_obj) == "string" then
          path_obj = Path:new(path_obj)
        elseif not path_obj.make_relative then
          -- Fallback if object lacks method
          path_obj = Path:new(tostring(path_obj))
        end

        local rel = path_obj:make_relative(VAULT_PATH)
        if rel:match("^" .. DAILY_NOTES_FOLDER) then
          is_daily = true
        end
      end
